...
contract Caller is EIP712("Caller", "1"), ERC2771Context(address(this)) {
    ...
    /// @return returnData The data returned by the call.
    function callSigned(address sender, address target, bytes calldata data, uint256 deadline, bytes32 r, bytes32 sv)
        public
        payable
        returns (bytes memory returnData)
    {
        ...
        bytes memory encoded =
            abi.encode(CALL_SIGNED_TYPE_HASH, sender, target, dataHash, msg.value, currNonce, deadline);
        // forge-lint: disable-next-line(asm-keccak256)
    ...
    function _getAuthorizedSet(address sender) internal view returns (EnumerableSet.AddressSet storage authorizedSet) {
