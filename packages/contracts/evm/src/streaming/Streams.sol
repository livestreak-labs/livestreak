...
abstract contract Streams {
    ...
    event StreamReceiverSeen(bytes32 indexed receiversHash, uint256 indexed accountId, StreamConfig config);
    ...
    event ReceivedStreams(uint256 indexed accountId, IERC20 indexed erc20, uint128 amt, uint32 receivableCycles);
    ...
    function _receiveStreams(uint256 accountId, IERC20 erc20, uint32 maxCycles) internal returns (uint128 receivedAmt) {
    ...
    function _receiveStreamsResult(uint256 accountId, IERC20 erc20, uint32 maxCycles)
        internal
        view
        returns (uint128 receivedAmt, uint32 receivableCycles, uint32 fromCycle, uint32 toCycle, int128 amtPerCycle)
    {
        unchecked {
            ...
            mapping(uint32 cycle => AmtDelta) storage amtDeltas = _streamsStorage().states[erc20][accountId].amtDeltas;
            // slither-disable-next-line timestamp
            for (uint32 cycle = fromCycle; cycle < toCycle; cycle++) {
    ...
    function _receivableStreamsCycles(uint256 accountId, IERC20 erc20) internal view returns (uint32 cycles) {
        unchecked {
    ...
    {
        {
            ...
            historyHashes = _verifyStreamsHistory(historyHash, streamsHistory, sender.streamsHistoryHash);
        ...
        uint32[2 ** 32] storage nextSqueezed = _streamsStorage().states[erc20][accountId].nextSqueezed[senderId];
    ...
    function _verifyStreamsHistory(
        ...
        for (uint256 i = 0; i < streamsHistory.length; i++) {
            ...
            historyHash = _hashStreamsHistory(historyHash, streamsHash, historyEntry.updateTime, historyEntry.maxEnd);
        }
    ...
    function _squeezedAmt(
        ...
        unchecked {
            ...
            for (; idx < receivers.length; idx++) {
                ...
                (uint32 start, uint32 end) = _streamRange(receiver, updateTime, maxEnd, squeezeStartCap, squeezeEndCap);
    ...
    {
        StreamsState storage state = _streamsStorage().states[erc20][accountId];
        return (state.streamsHash, state.streamsHistoryHash, state.updateTime, state.balance, state.maxEnd);
    }
    ...
    function _balanceAt(uint256 accountId, IERC20 erc20, StreamReceiver[] memory currReceivers, uint32 timestamp)
        internal
        view
        ...
        unchecked {
            ...
            for (uint256 i = 0; i < receivers.length; i++) {
                ...
                (uint32 start, uint32 end) = _streamRange({
                    receiver: receiver, updateTime: lastUpdate, maxEnd: maxEnd, startCap: lastUpdate, endCap: timestamp
                });
    ...
    function _setStreams(
        ...
        unchecked {
            ...
            uint32 newMaxEnd;
            {
                ...
                // forge-lint: disable-next-line(unsafe-typecast)
                int128 currBalance =
                    int128(_calcBalance(state.balance, lastUpdate, currMaxEnd, currReceivers, _currTimestamp()));
                ...
                _updateReceiverStates(
                    _streamsStorage().states[erc20], currReceivers, lastUpdate, currMaxEnd, newReceivers, newMaxEnd
                );
            }
            ...
            state.streamsHistoryHash = _hashStreamsHistory(streamsHistory, newStreamsHash, _currTimestamp(), newMaxEnd);
    ...
    function _verifyStreamsReceivers(StreamReceiver[] memory currReceivers, StreamsState storage state) private view {
    ...
    function _calcMaxEnd(uint128 balance, StreamReceiver[] memory receivers, uint32 hint1, uint32 hint2)
        private
        view
    ...
    function _isBalanceEnough(uint256 balance, uint256[] memory configs, uint256 configsLen, uint256 maxEnd)
        private
        view
    ...
    function _addConfig(uint256[] memory configs, uint256 configsLen, StreamReceiver memory receiver)
        private
        view
        returns (uint256 newConfigsLen)
    {
        ...
        (uint32 start, uint32 end) = _streamRangeInFuture(receiver, _currTimestamp(), type(uint32).max);
        // slither-disable-next-line incorrect-equality,timestamp
        if (start == end) {
            return configsLen;
        }
    ...
    function _hashStreams(StreamReceiver[] memory receivers) internal pure returns (bytes32 streamsHash) {
    ...
    function _hashStreamsHistory(bytes32 oldStreamsHistoryHash, bytes32 streamsHash, uint32 updateTime, uint32 maxEnd)
        internal
        pure
    ...
    function _updateReceiverStates(
        ...
        while (true) {
            ...
            if (pickCurr && pickNew) {
                if (
                    currRecv.accountId != newRecv.accountId || currRecv.config.amtPerSec() != newRecv.config.amtPerSec()
                ) {
            ...
            if (pickCurr && pickNew) {
                ...
                (uint32 currStart, uint32 currEnd) = _streamRangeInFuture(currRecv, lastUpdate, currMaxEnd);
                (uint32 newStart, uint32 newEnd) = _streamRangeInFuture(newRecv, _currTimestamp(), newMaxEnd);
                ...
                (uint32 start, uint32 end) = _streamRangeInFuture(newRecv, _currTimestamp(), newMaxEnd);
    ...
    function _addDeltaRange(StreamsState storage state, uint32 start, uint32 end, int256 amtPerSec) private {
        // slither-disable-next-line incorrect-equality,timestamp
        if (start == end) {
            return;
        }
    ...
    /// @param amtPerSec The streaming rate
    function _addDelta(mapping(uint32 cycle => AmtDelta) storage amtDeltas, uint256 timestamp, int256 amtPerSec)
        private
    {
        unchecked {
    ...
    function _streamedAmt(uint256 amtPerSec, uint256 start, uint256 end) private view returns (uint256 amt) {
