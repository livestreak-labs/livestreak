...
`packages/contracts` is Foundry-first Solidity plus wagmi-generated ABIs:

```text
packages/contracts/
  src/                Foundry Solidity — source of truth for chain behavior
  generated/          wagmi-generated ABIs and types (npm run gen)
  wagmi.config.ts     foundry plugin config
  script/             Deploy.s.sol
  docs/
    architecture.md                 this file
    streamed-funding-explained.md   the streamed-funding mechanism (Board on Vault)
```

| Layer | Owns | Does not own |
| --- | --- | --- |
| `src/` (Solidity) | Market registry, vaults, funding streams, LVST, steward hooks, events | UI, polling, panel projection, bookmaker strategy |
| `generated/` (wagmi) | ABIs and contract types from forge artifacts | Business workflows, read/write orchestration, deployment scripts |
| Host AA edge | UserOp submission proxy, paymaster endpoint, 4337 execution support | Vault math, market indexing, contract ABIs |

Downstream packages import **`@livestreak/contracts`** for wagmi-generated ABIs. They own transport, polling, and domain mapping (e.g. `packages/options/src/read/contracts/`).
...
**v0 locked law:** market ids are sequential `bytes32(uint256(n))` starting at `n = 1`. Same title/stream may register multiple markets; each gets a new id. Hash-based ids from observer/run metadata are deferred.
...
packages/contracts/src/
  market/       MarketRegistry
  bookmaker/    BookmakerRegistry
  vault/        Side, Vault (+ Board), VaultFactory, BondingBoard
  token/        LvstToken
  steward/      StewardRegistry
  streaming/    DripsStreaming, Streams, Managed, Caller, IDrips, drivers/AddressDriver
  aa/           AAImports.sol, LiveStreakPaymaster.sol
...
TypeScript boundary: wagmi `generated/contracts.ts`. Deploy addresses: `deploy/output/<chain>.json`.
